      ...
      <el-col :span="8">
        <el-card shadow="never">
          ...
          <div class="flex items-center gap-4">
            <img class="w-20 h-20 rounded-full object-cover border-2 border-gray-200" :src="avatar || Avatar"
...
const onAvatarSelect = async (file: UploadFile) => {
  try {
    ...
    if (result.success && result.data) {
      //前端预览地址
      avatar.value = result.data.previewUrl
      //数据库存储地址
      form.value.avatar = result.data.databaseUrl
    }
    else {
...
const onSave = async () => {
  ...
  const result = await updateUser(form.value)
  if (result.success && result.data) {
...
const init = () => {
  ...
  avatar.value = useAvatar().customAvatar(userStore.getUser?.avatar)
}
onMounted(() => {
  init()
})
